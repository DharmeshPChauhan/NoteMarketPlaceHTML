            ...
            <div class="row">
                <div class="col-md-12 table-responsive">
                    <table class="table table-hover" id="country-info-table">
                        ...
                        <tbody>
                            <?php
                                $select_country_query = "SELECT * FROM countries ORDER BY country_date_added DESC";
                                $check_select_country_query = mysqli_query($connection, $select_country_query);

                                if(!$check_select_country_query){
                                    echo ("QUERY FAILED " . mysqli_connect_error($connection));
                                }
                                $sr_no = 1;
                                $active = "Yes";
                                while($row = mysqli_fetch_assoc($check_select_country_query)){
                                    $country_id = $row['country_id'];
                                    $country_name = $row['country_name'];
                                    $country_code = $row['country_code'];
                                    $is_country_active = $row['is_country_active'];
                                    $country_date_added = $row['country_date_added'];
                                    $country_added_by_user = $row['country_added_by_user'];
                                    $country_date_edited = $row['country_date_edited'];
                                    $country_edited_by_user = $row['country_edited_by_user'];
                                    if ($is_country_active === 'true'){
                                        $active = "Yes";
                                    }
                                    else{
                                        $active = "No";
                                            ...
                                            <td>
                                                <div class='action-img'>
                                                    <div class='edit-country'>
                                                        <a href='include/edit_country.php?country_id={$country_id}'><img src='images/Dashboard/edit.png' alt='Edit' class='img-risponsive'></a>
                                                    </div>
