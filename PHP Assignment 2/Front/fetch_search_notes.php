    ...
    if(isset($_POST["type"])) {
        ...
        if(isset($_POST['search_rating'])) {
            ...
            if(!empty($search_rating))
                $select_note_sql_1 .= " AND (note_id,$search_rating) IN (SELECT review_note_id, AVG(`review_rating`) FROM note_reviews WHERE is_review_active = 1 GROUP BY `review_note_id`)";
        }
