<?php
    ...
    function password_generate() {
        ...
        $pass.= substr(str_shuffle($data_alphabets_lower), 0, 3);
...
//        $password = md5($password);
        ...
        $sql = "INSERT INTO users (user_role_id, user_first_name, user_last_name, user_email_id, user_password)";
        $sql.= " VALUES(1,'{$first_name}','{$last_name}','{$email}','{$password}')";
        
        $result = query($sql);
        confirmQuery($result);
        
        $subject = "Note Marketplace - Email Verification";
        $msg = "Hello ".$first_name.",<br/><br/>
            Thank you for signing up with us. Please click on below link to verify your email address and to do login.<br/><br/>
            <a href='http://localhost/NotesMarketplace/Front/EmailVerification.php?email={$email}&name={$first_name}'>Email verification link</a><br/><br/>
            Regards,<br/>
            ...
            if(email_exists($email)) {
                ...
                if(row_count($result) == 1) {
                    ...
                    if($is_active === '1' and $is_user_email_verified === '1') {
                            ...
                            echo "<script>alert('Your password has been changed successfully and newly generated password is sent on your registered email address.');window.location.href = 'login.php';</script>";
...
//            if(is_uploaded_file($_FILES['note_pdf']['tmp_name']))
    ...
    function change_password() {
        ...
        if(isset($_POST['change_pass_submit'])) {
            ...
            if(password_verify($old_pass, $db_pass) and $new_pass === $confirm_pass) {
                
                $new_password = password_hash($new_pass, PASSWORD_BCRYPT, array('cost' => 12));
                
                $change_password_sql = "UPDATE users SET user_password = '{$new_password}', modified_date = now(), modified_by = $the_user_id WHERE user_id = $the_user_id";
