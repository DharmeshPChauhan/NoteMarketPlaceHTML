...
<?php
    if(isset($_GET['country_id'])){
        ...
        $select_country_query = "SELECT * FROM countries WHERE country_id = {$country_id_edit}";
        ...
        if(isset($_POST['country_update'])){
            ...
            $edit_country_query = "UPDATE countries SET country_name = '{$country_name}', country_code = '{$country_code}', is_country_active = 'true', country_date_edited = now(), country_edited_by_user = 'admin' WHERE country_id = {$country_id_edit}";

            $check_edit_country_query = mysqli_query($connection, $edit_country_query);

            if(!$check_edit_country_query){
                echo ("QUERY FAILED " . mysqli_connect_error($connection));
            }
            else{
...
<head>
    ...
    <!-- Title -->
    <title>Add Country - Notes Marketplace</title>
